...
const renderMeta = (item) => {
  setText("[data-blog-label]", item?.label || "Work Blog");
  ...
  document.title = item?.title ? `${item.title} | Ved Panse` : "Work Blog Post | Ved Panse";
};
...
const listWorkBlogFiles = async () => {
  ...
  return indexData.map((file) => (file.startsWith(WORK_BLOGS_DIR) ? file : `${WORK_BLOGS_DIR}/${file}`));
};

const loadWorkBlogSummaries = async () => {
  const files = await listWorkBlogFiles();
...
const initWorkBlogPost = async () => {
  ...
  const response = await fetch(`${WORK_BLOGS_DIR}/${slug}.md`);
  let summaries = [];
  try {
    summaries = await loadWorkBlogSummaries();
  } catch {
    summaries = [];
  }
